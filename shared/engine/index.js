 ...
 */
export function createInitialState(players, rules = { winLength: 4, maxPlayers: 20, allowSpectators: true }) {
    return {
        board: [
            ['', '', ''],
            ['', '', ''],
 ...
 */
export function isCenterCell(state, row, col) {
 ...
 */
export function isAdjacentToFilledCell(state, row, col) {
    ...
    for (const [dr, dc] of directions) {
        const newRow = row + dr;
        const newCol = col + dc;
        if (isValidPosition(state, newRow, newCol) &&
 ...
 */
export function getExpansionEdges(state, row, col) {
    return {
        top: row === 0,
 ...
 */
export function shouldExpandBoard(state, row, col) {
...
export function expandBoard(state, row, col) {
    ...
    if (expandedEdges.bottom) {
        newState.board.push(Array(newState.boardSize.cols).fill(''));
        newState.boardSize.rows++;
    }
...
export function applyMove(state, playerId, row, col) {
    ...
    let expandedEdges = { top: false, bottom: false, left: false, right: false };
    let finalRow = row;
    let finalCol = col;
 ...
 */
export function startTurnTimer(state, timeLimit) {
    const newState = cloneState(state);
 ...
 */
export function clearTurnTimer(state) {
    const newState = cloneState(state);
 ...
 */
export function isTurnTimerExpired(state) {
